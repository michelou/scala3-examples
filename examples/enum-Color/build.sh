...
args() {
    ...
    if [[ $SCALA_CLI -eq 1 ]]; then
        COMMANDS="${COMMANDS/clean/clean_cli}"
        COMMANDS="${COMMANDS/compile/compile_cli}"
        COMMANDS="${COMMANDS/run/run_cli}"
    fi
    USE_CODE_RUNNER=0
    if [[ $SCALA_VERSION -eq 2 ]]; then
        ...
        SCALAC_CMD="$SCALA3_HOME/bin/scalac"
        SCALADOC_CMD="$SCALA3_HOME/bin/scaladoc"
        SCALAC_OPTS="-color never"
        [[ -f "$SCALA3_HOME/libexec/common" ]] && USE_CODE_RUNNER=1
    fi
    debug "Properties : USE_CODE_RUNNER=$USE_CODE_RUNNER USE_RC=$USE_RC"
    debug "Options    : SCALA_VERSION=$SCALA_VERSION TIMER=$TIMER VERBOSE=$VERBOSE"
...
clean_cli() {
    local scala_cli_opts=""
    if [[ $DEBUG -eq 1 ]]; then
        debug "\"$SCALA_CLI_CMD\" clean $scala_cli_opts"
    elif [[ $VERBOSE -eq 1 ]]; then
        echo "Clean project (scala-cli)"
    fi
    eval "$SCALA_CLI_CMD" clean $scala_cli_opts "$(mixed_path $SOURCE_SCALA_DIR)"
    [[ $? -eq 0 ]] || ( EXITCODE=1 && return 0 )
}
...
if [[ $(($cygwin + $mingw + $msys)) -gt 0 ]]; then
    ...
    [[ -n "$SCALA_CLI_HOME" ]] && SCALA_CLI_HOME="$(mixed_path $SCALA_CLI_HOME)"
...
unset SCALA_CLI_CMD
if [[ ! -x "$SCALA_CLI_HOME/scala-ci$EXE" ]]; then
    SCALA_CLI_CMD="$SCALA_CLI_HOME/scala-ci$EXE"
fi
