...
$SCALAC2_CMD = $Env:SCALA_HOME + $SEP + 'bin' + $SEP + 'scalac' + $BAT
if (! (Test-Path -PathType Leaf -Path $SCALAC2_CMD)) {
    Write-Error "Scala 2 compiler not found (check variable ""SCALA_HOME"")"
    Cleanup 1
}
$SCALAC3_CMD = $Env:SCALA3_HOME + $SEP + 'bin' + $SEP + 'scalac' + $BAT
if (! (Test-Path -PathType Leaf -Path $SCALAC3_CMD)) {
    Write-Error "Scala 3 compiler not found (check variable ""SCALA3_HOME"")"
    Cleanup 1
}
$SCALAC3_RC_CMD = $Env:SCALA3_RC_HOME + $SEP + 'bin' + $SEP + 'scalac' + $BAT
if (! (Test-Path -PathType Leaf -Path $SCALAC3_RC_CMD)) {
    ## optional
    $SCALAC3_RC_CMD = $null
}
...
Write-Debug "Options    : DEBUG=$DEBUG SCALA_VERSION=$SCALA_VERSION TIMER=$TIMER USE_RC=$USE_RC VERBOSE=$VERBOSE"
...
function Print-Help
{
    ...
    Write-Output "     -scala3     use Scala 2 distribution (default)"
...
function Compile
{
    ...
    if (Test-Path -PathType Container -Path $RESOURCES_DIR) {
...
function Run-CLI
{
    ...
    $CPATH = $(Build-Classpath) + $CLASSES_DIR
    $SCALA_CLI_OPTS = @('--main-class', """$MAIN_CLASS""", '--classpath', """$CPATH""")

    Write-Debug """$SCALA_CMD"" run $SCALA_CLI_OPTS"
    Write-Verbose "Execute Scala main class ""$MAIN_CLASS"" (scala-cli)"
    &"$SCALA_CMD" run $SCALA_CLI_OPTS
    if ($LASTEXITCODE -ne 0) {
        Write-Error "Failed to execute Scala main class ""$MAIN_CLASS"" (scala-cli)"
        Cleanup 1
    }
}
