...
set _JAVA_OPTS=-deprecation

set _USE_CODE_RUNNER=0
if %_SCALA_VERSION%==3 (
    if %_USE_RC%==1 if defined SCALA3_RC_HOME if exist "%SCALA3_RC_HOME%\bin\scalac.bat" (
        set "_SCALA3_CMD=%SCALA3_RC_HOME%\bin\scala.bat"
        set "_SCALAC3_CMD=%SCALA3_RC_HOME%\bin\scalac.bat"
        set "_SCALADOC3_CMD=%SCALA3_RC_HOME%\bin\scaladoc.bat"
        if exist "%SCALA3_RC_HOME%\libexec\common.bat" set _USE_CODE_RUNNER=1
    ) else (
        if exist "%SCALA3_HOME%\libexec\common.bat" set _USE_CODE_RUNNER=1
    )
    set _SCALAC_OPTS=-deprecation -feature -language:implicitConversions
) else (
...
if %_DEBUG%==1 (
    echo %_DEBUG_LABEL% Properties : _PROJECT_NAME=%_PROJECT_NAME% _PROJECT_VERSION=%_PROJECT_VERSION%
...
) else if %_VERBOSE%==1 ( echo Clean project ^(scala_cli^) 1>&2
)
call "%_SCALA_CLI_CMD%" clean %__CLI_OPTS% "%_SOURCE_SCALA_DIR%"
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to clean project ^(scala_cli^) 1>&2
    set _EXITCODE=1
    goto :eof
)
goto :eof
...
if exist "%_RESOURCES_DIR%\*" (
    if %_DEBUG%==1 ( echo %_DEBUG_LABEL% copy /y "%_RESOURCES_DIR%\*" "%_CLASSES_DIR%" 1>&2
...
set __CLI_OPTS=-O -deprecation %__CLI_OPTS% --classpath "!__CPATH:%USERPROFILE%=%%USERPROFILE%%!"
set __CLI_OPTS=%__CLI_OPTS% --compilation-output "%_CLASSES_DIR%"
...
echo %_JAVA_OPTS% -classpath "%__CPATH:\=\\%" -d "%_CLASSES_DIR:\=\\%" > "%__OPTS_FILE%"
...
set __CLI_OPTS=%__CLI_OPTS% --main-class "%_MAIN_CLASS%" --classpath "!__CPATH:%USERPROFILE%=%%USERPROFILE%%!"
if %_DEBUG%==1 ( set __CLI_OPTS=-v %__CLI_OPTS%
) else if %_VERBOSE%==1 ( set __CLI_OPTS=-v %__CLI_OPTS%
)
set __MAIN_ARGS=-- 1
...
call "%_SCALA_CMD%" %__SCALA_OPTS%
if not %ERRORLEVEL%==0 (
    echo %_ERROR_LABEL% Failed to execute Scala main class "%_MAIN_CLASS%" 1>&2
    set _EXITCODE=1
    goto :eof
)
if %_TASTY%==1 if %_USE_CODE_RUNNER%==0 (
    call :run_tasty
    if not !_EXITCODE!==0 goto :eof
)
goto :eof
