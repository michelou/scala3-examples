...
args() {
    ...
    if [[ $USE_RC -eq 1 ]]; then
        if [[ -x "$SCALA3_RC_HOME/bin/scalac" ]]; then
            SCALA3="$SCALA3_RC_HOME/bin/scala"
            SCALAC3="$SCALA3_RC_HOME/bin/scalac"
            SCALADOC3="$SCALA3_RC_HOME/bin/scaladoc"
        else
            warning "Scala 3 release candidate not found (use default version)"
            USE_RC=0
        fi
    fi
    local is_lamp=$($SCALA3 -version 2>&1 | grep "LAMP")
    [[ -n $is_lamp ]] || RUN_JAVA=_java

    debug "Options    : TIMER=$TIMER USE_RC=$USE_RC VERBOSE=$VERBOSE"
...
doc() {
    ...
    local is_required="$(action_required "$doc_timestamp_file" "$SOURCE_MAIN_DIR/" "*.scala")"
    ...
    #     echo $(mixed_path $f) >> "$sources_file"
    # done
    for f in $(find $SOURCE_SCALA_DIR/ -name *.scala 2>/dev/null); do
        echo $(mixed_path $f) >> "$sources_file"
    done
...
run_java() {
    ...
    local maven2_dir="$(dirname $(dirname $SCALA3))/maven2"
    local cpath=""
    while IFS= read -rd '' line; do
        ##echo "$line"
        cpath+="$line$PSEP"
    done < <(find "$maven2_dir" -name "*.jar" -print0)
    cpath+="$CLASSES_DIR"

    local java_opts_file="$(mixed_path $TARGET_DIR/java_opts.txt)"
    echo "-classpath $cpath" > $java_opts_file

    if [[ $DEBUG -eq 1 ]]; then
        debug "$JAVA_CMD \"@$java_opts_file\" $MAIN_CLASS $MAIN_ARGS"
    elif [[ $VERBOSE -eq 1 ]]; then
        echo "Execute Scala main class \"$MAIN_CLASS\"" 1>&2
    fi
    eval "$JAVA_CMD" "@$java_opts_file" $MAIN_CLASS $MAIN_ARGS
    if [[ $? -ne 0 ]]; then
        error "Failed to execute Scala main class \"$MAIN_CLASS\""
        cleanup 1
    fi
