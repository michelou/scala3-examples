...
if ($USE_RC) {
    ...
    if (Test-Path -PathType Leaf -Path $SCALAC_RC_CMD) {
        ...
        $SCALADOC_CMD = $Env:SCALA3_RC__HOME + $SEP + 'bin' + $SEP + 'scaladoc.bat'
    }
    else {
        Write-Warning "Scala 3 RC compiler not found (use default Scala 3 version)"
        $USE_RC = $false
    }
}
...
function Print-Help
{
    ...
    Write-Output "     -rc         use release candidate version if available"
